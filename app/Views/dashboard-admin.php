            ...
            <div class="container-fluid full-width">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white border-0 d-flex flex-column flex-md-row justify-content-between align-items-center py-3">
                        ...
                        <form method="GET" action="<?= site_url('dashboard/admin') ?>" class="search-container position-relative">
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <?php if ($row['status'] == 'Selesai'): ?>
                                                    <!-- Kalau sudah selesai -->
                                                    <button class="btn btn-sm btn-outline-secondary rounded" disabled data-bs-toggle="tooltip" title="Sudah selesai disposisi">
                                                        <i class="fas fa-paper-plane"></i>
                                                    </button>
                                                    <button class="btn btn-sm btn-outline-secondary rounded" disabled data-bs-toggle="tooltip" title="Sudah selesai rekomendasi">
                                                        <i class="fas fa-clipboard-check"></i>
                                                    </button>
                                                <?php elseif (!empty($row['disposisi_id'])): ?>
                                                    <!-- Kalau sudah ada disposisi, rekomendasi aktif -->
                                                    <button class="btn btn-sm btn-outline-secondary rounded" disabled data-bs-toggle="tooltip" title="Sudah didisposisi">
                                                        <i class="fas fa-paper-plane"></i>
                                                    </button>
                                                    <button class="btn btn-sm btn-outline-success rounded" data-bs-toggle="tooltip" title="Buat Rekomendasi"
                                                        onclick="window.location.href='<?= site_url('rekomendasi/' . $row['disposisi_id']) ?>'">
                                                        <i class="fas fa-clipboard-check"></i>
                                                    </button>
                                                <?php else: ?>
                                                    <!-- Kalau belum ada disposisi, disposisi aktif -->
                                                    <button class="btn btn-sm btn-outline-primary rounded" data-bs-toggle="tooltip" title="Buat Disposisi"
                                                        onclick="window.location.href='<?= site_url('disposisi/' . $row['id']) ?>'">
                                                        <i class="fas fa-paper-plane"></i>
                                                    </button>
                                                    <button class="btn btn-sm btn-outline-secondary rounded" disabled data-bs-toggle="tooltip" title="Belum bisa rekomendasi">
                                                        <i class="fas fa-clipboard-check"></i>
                                                    </button>
                                                <?php endif; ?>
                                            </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Data from PHP
    const rekomendasiStatistisiData = <?= json_encode($rekomendasiStatistisiData) ?>;
    const rekomendasiPrakomData = <?= json_encode($rekomendasiPrakomData) ?>;
    const existingStatistisi = <?= json_encode($existingStatistisi) ?>;
    const existingPrakom = <?= json_encode($existingPrakom) ?>;

    // Labels from PHP (tabel tingkat_kompetensi)
    const existingPrakomLabels = <?= json_encode($existingPrakomLabels) ?>;
    const existingStatistisiLabels = <?= json_encode($existingStatistisiLabels) ?>;
    const rekomendasiPrakomLabels = <?= json_encode($rekomendasiPrakomLabels) ?>;
    const rekomendasiStatistisiLabels = <?= json_encode($rekomendasiStatistisiLabels) ?>;
    ...
    // Chart 1 - Rekomendasi Statistisi
    new Chart(document.getElementById('statistisiChart').getContext('2d'), {
        type: 'bar',
        data: {
            labels: rekomendasiStatistisiLabels,  // Use the labels from PHP (tabel tingkat_kompetensi)
            datasets: [{
                label: 'Rekomendasi Statistisi',
                data: rekomendasiStatistisiData,  // Use the data from PHP
    ...
    // Chart 2 - Rekomendasi Prakom
    new Chart(document.getElementById('prakomChart').getContext('2d'), {
        type: 'bar',
        data: {
            labels: rekomendasiPrakomLabels,  // Use the labels from PHP (tabel tingkat_kompetensi)
            datasets: [{
                label: 'Rekomendasi Prakom',
                data: rekomendasiPrakomData,  // Use the data from PHP
    ...
    // Chart 3 - Existing Statistisi
    new Chart(document.getElementById('existingStatistisiChart').getContext('2d'), {
        type: 'bar',
        data: {
            labels: existingStatistisiLabels,  // Use the labels from PHP (tabel tingkat_kompetensi)
            datasets: [{
                label: 'Existing Statistisi',
                data: Object.values(existingStatistisi),  // Use the data from PHP
    ...
    // Chart 4 - Existing Prakom
    new Chart(document.getElementById('existingPrakomChart').getContext('2d'), {
        type: 'bar',
        data: {
            labels: existingPrakomLabels,  // Use the labels from PHP (tabel tingkat_kompetensi)
            datasets: [{
                label: 'Existing Prakom',
                data: Object.values(existingPrakom),  // Use the data from PHP
